...
class AdministradorPublicacionesPage extends StatefulWidget {
  @override
  _AdministradorPublicacionesPage createState() =>
      _AdministradorPublicacionesPage();
}

class _AdministradorPublicacionesPage
    extends State<AdministradorPublicacionesPage> {
  String fil = 'Aprobado';
          ...
          child: GestureDetector(
            ...
            child: Container(
                ...
                decoration: BoxDecoration(
                  color: _ADMPageProvider.Gpubpage == page
                      ? Color(0Xff2056a1)
                      : (_ADMPageProvider.Gpubpage ==
                                  'Publicaciones de usuarios' &&
                              page == 'Publicaciones de usuarios')
                          ? Color(0Xff2056a1)
                          : (_ADMPageProvider.Gpubpage == 'Crear Publicacion' &&
                                  page == 'Crear Publicacion')
                              ? Color(0Xff2056a1)
                              : (_ADMPageProvider.Gpubpage == 'Crear Anuncio' &&
                                      page == 'Crear Anuncio')
                                  ? Color(0Xff2056a1)
                                  : Colors.white,
                  border: Border.all(
      ...
      child: OutlinedButton(
        style: ButtonStyle(
            backgroundColor: MaterialStateProperty.all<Color>(
                fil == val ? Color(0Xff2056a1) : Colors.white),
            foregroundColor: MaterialStateProperty.all<Color>(
                fil == val ? Colors.white : Colors.black)),
        onPressed: () {
          setState(() {
            fil = val;
          });
          ...
          child: Row(
            ...
            children: [
              ...
              MouseRegion(
                ...
                child: GestureDetector(
                  onTap: () {
                    FirestoreAPI().Pubchangestate(true, pub);
                  },
                  child: Container(
              ...
              MouseRegion(
                ...
                child: GestureDetector(
                  onTap: () {
                    FirestoreAPI().Pubchangestate(false, pub);
                  },
                  child: Container(
                    width: MediaQuery.of(context).size.width * 0.125,
                    height: MediaQuery.of(context).size.height * 0.036,
                    child: Text(
                      pub.estado == "Rechazado" ? "Aprobar" : "Rechazar",
                      style: TextStyle(
                          fontSize: MediaQuery.of(context).size.width * 0.015,
                          fontWeight: FontWeight.w900,
                          color: pub.estado == "Rechazado"
                              ? Colors.green
                              : Colors.red),
                    ),
                  ),
  ...
  contenido(BuildContext context) {
    final _ADMPageProvider =
        Provider.of<AdmPageProvider>(context, listen: true);
    return _ADMPageProvider.Gpubpage == 'Publicaciones de usuarios'
        ? ScreenPublicaciones(context)
        : _ADMPageProvider.Gpubpage == 'Crear Publicacion'
            ? AdministradorCrearPublicacionesPage()
            : _ADMPageProvider.Gpubpage == 'Crear Anuncio'
                ? AdministradorCrearAnunciosPage()
                : Container();
  }
