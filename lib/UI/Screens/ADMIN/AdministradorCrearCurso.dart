...
class AdministradorCrearCursosPage extends StatefulWidget {
  @override
  _AdministradorCrearCursosPage createState() => _AdministradorCrearCursosPage();
}

class _AdministradorCrearCursosPage extends State<AdministradorCrearCursosPage> {
  ...
  void uploadPost() async {
    ...
    fb.StorageReference storageRef = fb.storage().ref("cursos").child(timekey+imgPortada.name);
    ...
    if(img1!=null){
      ...
      fb.StorageReference storageRef1 = fb.storage().ref("cursos").child(timekey1+img1.name);
    ...
    if(img2!=null){
      var timekey2 = (DateTime.now().day+DateTime.now().month+DateTime.now().year+
          DateTime.now().hour+DateTime.now().minute+DateTime.now().second).toString();
      fb.StorageReference storageRef2 = fb.storage().ref("cursos").child(timekey2+img2.name);
    ...
    if(img3!=null){
      var timekey3 = (DateTime.now().day+DateTime.now().month+DateTime.now().year+
          DateTime.now().hour+DateTime.now().minute+DateTime.now().second).toString();
      fb.StorageReference storageRef3 = fb.storage().ref("cursos").child(timekey3+img3.name);
      fb.UploadTaskSnapshot uploadTaskSnapshot3 = await storageRef3.put(img3).future;

      Uri urlImg3 = await uploadTaskSnapshot3.ref.getDownloadURL();
      curso.img3=urlImg3.toString();
    }
    if(imgponiente!=null){
      var timekey4 = (DateTime.now().day+DateTime.now().month+DateTime.now().year+
          DateTime.now().hour+DateTime.now().minute+DateTime.now().second).toString();
      fb.StorageReference storageRef4 = fb.storage().ref("cursos").child(timekey4+imgponiente.name);
      fb.UploadTaskSnapshot uploadTaskSnapshot4 = await storageRef4.put(imgponiente).future;
            ...
            SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  FormNomCurso('Nombre del Curso',ScreenW*0.2,ScreenH*0.05),
                  SizedBox(
                    height: MediaQuery.of(context).size.height*0.03,
                  ),
                  Column(
                    children: [
                          ...
                          CircleAvatar(
                              ...
                              child: Container(
                                decoration: BoxDecoration(
                                  ...
                                  image: DecorationImage(
                                      fit: BoxFit.cover,
                                      image: imgponiente!=null? NetworkImage(pathponiente.url):AssetImage("assets/images/APP - Pantalla Foto Perfil Referencial.png")
                                  ),
                                ),)),
                          Positioned(
                            child: MouseRegion(
                      ...
                      Padding(
                        ...
                        child: FormPoniente('Poniente',ScreenW*0.2,ScreenH*0.05),
                      ),
                    ],
                  ),
                  Row(
                    ...
                    children: [
                      Container(
                          ...
                          child: Center(
                            child: Text((_date!=null && _time!=null)?'Fecha: '+_date+' '+_time:"Selecciona Fecha",
                              style: TextStyle(
                                  color: Colors.white,
                                  fontSize: MediaQuery.of(context).size.width*0.01
                              ),),
                  ...
                  FormInfoCurso("Información del Curso",ScreenW*0.4,ScreenH*0.4),
                  ...
                  MouseRegion(
                    ...
                    child: Row(
                      ...
                      children: [
                        GestureDetector(
                          onTap: (){
                            if(_pubTit!=null && _pubText!=null && _date!=null && _time!=null && _link!=null)
                            {
                              curso= new Curso(
                                  id: _pubTit.substring(0,3)+'admin124563258'.substring(5),
                                  Autorid: 'admin',
                                  titulo: _pubTit,
                                  poniente: _poniente,
                                  link: _link,
